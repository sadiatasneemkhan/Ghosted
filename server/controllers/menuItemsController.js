...
export async function getMenuItemByMenuItemId(id) {
  ...
  export async function createMenuItem(restaurantId, name, description, price, isAvailable, categoryId, image, prepTime) {
    try {
      const [result] = await pool.query(
        `INSERT INTO menu_items (restaurant_id, name, description, price, is_available, category_id, image, prep_time) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [restaurantId, name, description, price, isAvailable, categoryId, image, prepTime]
      );
  ...
  export async function deleteMenuItem(menuItemId, restaurantId) {
    try {
      ...
      if (ownershipResult.length === 0) {
        throw new Error('Menu item not found or does not belong to the specified restaurant');
      }
  ...
  export async function updateMenuItem(menuItemId, name, description, price, isAvailable, categoryId, image, prepTime) {
    try {
      const [result] = await pool.query(
        `UPDATE menu_items SET name = ?, description = ?, price = ?, is_available = ?, category_id = ?, image = ?, prep_time = ?
         WHERE menu_item_id = ?`,
        [name, description, price, isAvailable, categoryId, image, prepTime, menuItemId]
      );
