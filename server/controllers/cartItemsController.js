...
export async function getCartItemByCartItemId(id) {
  ...
  export async function getCartItemsByCartId(id) {
    const [rows] = await pool.query(
      'SELECT cart_item_id, cart_id,item_id,quantity FROM cart_items WHERE cart_id = ?'
      ,[id]
      );
    return rows;
  }
  ...
  export async function updateCartItemQuantity(cartItemId, cartId, newQuantity) {
    try {
      ...
      if (result.affectedRows === 0) {
        throw new Error('Cart item not found, does not belong to this cart, or quantity unchanged');
      }
  ...
  export async function removeCartItem(cartItemId, cartId) {
    try {
      ...
      if (result.affectedRows === 0) {
        throw new Error('Cart item not found, already removed, or does not belong to this cart');
      }
