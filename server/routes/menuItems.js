...
router.post('/', async (req, res) => {
  try {
    const { restaurantId, name, description, price, isAvailable, categoryId, image, prepTime } = req.body;
    const result = await createMenuItem(restaurantId, name, description, price, isAvailable, categoryId, image, prepTime);
...
router.put('/:menu_item_id', async (req, res) => {
  try {
    const menu_item_id = req.params.menu_item_id;
    const { name, description, price, isAvailable, categoryId, image, prepTime } = req.body;
    const result = await updateMenuItem(menu_item_id, name, description, price, isAvailable, categoryId, image, prepTime);
    if (result.success) {
...
router.delete('/:menu_item_id/:restaurant_id', async (req, res) => {
  try {
    ...
    if (result.success) {
      ...
      res.status(404).send('Menu item not found or does not belong to the specified restaurant');
    }
  } catch (error) {
